...
public class Main implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        KeyMapping openScreenKeyBind = KeyBindingHelper.registerKeyBinding(
                new KeyMapping(MOD_ID + ".key_bind.open_screen", InputConstants.Type.KEYSYM, GLFW.GLFW_KEY_J, KeyMapping.Category.MISC));
            ...
            public void onStartTick(@NonNull Minecraft client) {
                ...
                if (openScreenKeyBind.consumeClick()) {
                    if (SongLoader.loadingSongs) {
                        client.gui.getChat().addMessage(Component.translatable(Main.MOD_ID + ".still_loading").copy().withStyle(ChatFormatting.RED));
        ...
        ClientTickEvents.START_WORLD_TICK.register(world -> {
            for (ClientTickEvents.StartWorldTick listener : new ArrayList<>(TICK_LISTENERS)) listener.onStartTick(world);
        });

        ClientCommandRegistrationCallback.EVENT.register((dispatcher, unused) -> DiscjockeyCommand.register(dispatcher));

        ClientLoginConnectionEvents.DISCONNECT.register((unusedHandler, unusedClient) -> {
            PREVIEWER.stop();
            SONG_PLAYER.stop();
        });
